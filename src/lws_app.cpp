    ...
    if (LWSOptions::runTPE || buttonStepTPE) {
        ...
        if (LWSOptions::useSobolev) {
            if (LWSOptions::useMultigrid) {
                good_step = tpeSolver->StepSobolevLSIterative(0, useBackproj, sobolevGradients);
            } else {
            ...
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++) {
                        ...
                        outPos << "v " << b[i].position.x << " " << b[j].position.y << " " << b[k].position.z
                            << endl;
                        outTan << "v " << b[i].tangent.x << " " << b[j].tangent.y << " " << b[k].tangent.z
                            << endl;
                    }
            nBoxes++;
...
void LWSApp::centerizeLoopBarycenter(PolyCurveNetwork* curves) {
    const Vector3 center = curves->Barycenter();
...
void LWSApp::initSolver() {
    ...
    //     std::cout << "Vertex " << i << " has angle " << acos(dot(v1, v2)) << " (dot " << dot(v1, v2) << ")"
    //     << std::endl;
    //   }
    // }
    ...
    if (!tpeSolver) {
        ...
        for (ObstacleData& data : sceneData.obstacles) {
            std::cout << "Adding scene obstacle from " << data.filename << " (weight " << data.weight << ")"
                << std::endl;
            AddMeshObstacle(data.filename, Vector3::zero(), beta - alpha, data.weight);
        }

        for (PlaneObstacleData& data : sceneData.planes) {
            std::cout << "Adding plane obstacle (center " << data.center << ", normal " << data.normal
                << ", weight " << data.weight << std::endl;
            AddPlaneObstacle(data.center, data.normal, beta - alpha, data.weight);
        }

        for (const std::string& surfaceName : sceneData.surfacesToShow) {
            VisualizeMesh(surfaceName);
        }

        for (const PotentialData& data : sceneData.extraPotentials) {
            switch (data.type) {
                ...
                case PotentialType::VectorField:
                    if (data.extraInfo == "constant") {
                        std::cout << "Adding constant vector field potential (weight = " << data.weight << ")"
                            << std::endl;
                        tpeSolver->potentials.push_back(
                            new VectorFieldPotential(data.weight, new ConstantVectorField(Vector3{1, 0, 1})));
                    } else if (data.extraInfo == "circular") {
                        std::cout << "Adding circular vector field potential (weight = " << data.weight << ")"
                            << std::endl;
                        tpeSolver->potentials.push_back(
                            new VectorFieldPotential(data.weight, new CircularVectorField()));
                    } else if (data.extraInfo == "interesting") {
                        std::cout << "Adding interesting vector field potential (weight = " << data.weight
                            << ")" << std::endl;
                        tpeSolver->potentials.push_back(
                            new VectorFieldPotential(data.weight, new InterestingVectorField()));
                    } else {
...
void LWSApp::VisualizeMesh(const std::string& objName) const {
    ...
    const std::string name = polyscope::guessNiceNameFromPath(objName);
    polyscope::registerSurfaceMesh(name, geometry->inputVertexPositions, mesh->getFaceVertexList(),
                                   polyscopePermutations(*mesh));
}

void LWSApp::AddMeshObstacle(const std::string& objName, const Vector3& center, double p, double weight) {
    std::unique_ptr<HalfedgeMesh> mesh;
    std::unique_ptr<VertexPositionGeometry> geometry;
    std::tie(mesh, geometry) = loadMesh(objName);
...
void LWSApp::AddPlaneObstacle(const Vector3& center, const Vector3& normal, double p, double weight) {
...
void LWSApp::AddSphereObstacle(const Vector3& center, double radius) {
    int numObs = tpeSolver->obstacles.size();
...
void LWSApp::WriteImplicitSurface(const char* surfaceMeshName) {
    ...
    for (size_t i = 0; i < nVerts; i++) {
        glm::vec3 pos = mesh->vertexPositions.getValue(i);
    ...
    for (size_t i = 0; i < nFaces; i++) {
        size_t nInFace = mesh->faceIndsStart[i];
        objFile << "f ";
        for (size_t j = 0; j < nInFace; j++) {
            objFile << mesh->faceIndsEntries[i + j] + 1;
            if (j < nInFace - 1) {
                objFile << " ";
            }
        }
...
void LWSApp::DisplayPlane(const Vector3& center, const Vector3& normal, const std::string& name) {
    Vector3 v1{1, 0, 0};
...
void LWSApp::DisplayWireSphere(const Vector3& center, double radius, const std::string& name) {
...
void LWSApp::DisplayCurveNetwork(const PolyCurveNetwork* curves, const std::string& name) {
    ...
    for (const auto& v : curves->Vertices()) {
        // Add interior edges and vertices
        const Vector3 p = curves->Position(v);
        nodes.push_back(glm::vec3{p.x, p.y, p.z});
    }
    for (const auto& e : curves->Edges()) {
        edges.push_back({(size_t)e->prevVert->GlobalIndex(), (size_t)e->nextVert->GlobalIndex()});
    }

    polyscope::registerCurveNetwork(name, nodes, edges);
    polyscope::getCurveNetwork(name)->setRadius(0.015f);
}

void LWSApp::DisplayCyclicList(const std::vector<Vector3>& positions, const std::string& name) {
    std::vector<std::array<size_t, 2>> edges;
    ...
    polyscope::getCurveNetwork(name)->setRadius(0.01f);
}

void LWSApp::DisplaySobolevGradients(const PolyCurveNetwork* curves,
                                     const Eigen::MatrixXd& sobolevGradients) {
    if (!sobolevGradients.size()) {
        return;
    }
    assert(sobolevGradients.size() == curves->NumVertices() * 3);

    // [curves] -> [curveNodes]
    const std::vector<Vector3> curveNodes = curves->AllPositions();
    assert(sobolevGradients->rows() == curveNodes.size());

    // [curveNodes] -> [gradSegments]
    // NOTE: [curveNodes]'s size is not changed since loading!
    static std::vector<Vector3> gradSegments(curveNodes.size() * 2);
    for (size_t i = 0; i < curveNodes.size(); ++i) {
        const auto& curveNode = curveNodes[i];
        gradSegments[2 * i] = curveNode;
        gradSegments[2 * i + 1] = curveNode + 0.5 * SelectRow(sobolevGradients, i);
    }
...
void LWSApp::processFileOBJ(const std::string& filename) {
    ...
    for (const Vertex& v : mesh->vertices()) {
        const size_t index = indices[v];
        all_positions[index] = geom->vertexPositions[v];
    }

    for (const BoundaryLoop& b : mesh->boundaryLoops()) {
...
void LWSApp::processSceneFile(const std::string& filename) {
...
LWS::LWSApp* LWS::LWSApp::instance = nullptr;

bool endsWith(const std::string& str, const std::string& suffix) {
    return str.size() >= suffix.size() && str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0;
}

void processFile(LWS::LWSApp* app, const string& filename) {
...
void customWindow() { LWS::LWSApp::instance->customWindow(); }

int main(int argc, char** argv) {
    int default_threads = omp_get_max_threads();
    int used_threads = default_threads / 2 + 2;
    omp_set_num_threads(used_threads);
    std::cout << "OMP autodetected " << default_threads << " max threads; using " << used_threads
        << std::endl;
    ...
    auto* app = new LWS::LWSApp();
    LWS::LWSApp::instance = app;

    std::cout << "Using Eigen version " << EIGEN_WORLD_VERSION << "." << EIGEN_MAJOR_VERSION << "."
    ...
    app->initSolver();
    std::cout << "Set up solver done!" << std::endl;

    if (obstacleFiles) {
        for (const string& obsFile : obstacleFiles) {
            app->AddMeshObstacle(obsFile, Vector3::zero(), 3, 1);
        }
    }
    if (visualizeFiles) {
        for (const string& visFile : visualizeFiles) {
            app->VisualizeMesh(visFile);
        }
    }

    // app->AddPlaneObstacle(Vector3{-2, 0, 0}, const Vector3&{1, 0, 0});
    // app->AddPlaneObstacle(Vector3{2, 0, 0}, const Vector3&{-1, 0, 0});
