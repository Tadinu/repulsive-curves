...
void SpatialTree::TPEGradientBarnesHut(const PolyCurveNetwork* curveNetwork, SpatialTree* root,
                                       Eigen::MatrixXd& output, double alpha, double beta) {
...
#pragma omp parallel firstprivate(partialOutput) shared(root, output)
    {
#pragma omp for
        for (CurveVertex* i_pt : curveNetwork->Vertices()) {
...
double SpatialTree::TPEnergyBH(const PolyCurveNetwork* curveNetwork, SpatialTree* root, double alpha,
                               double beta) {
    double fullSum = 0;

#pragma omp parallel for reduction(+ : fullSum) shared(root)
    // Loop over all vertices and add up energy contributions
    for (const CurveVertex* i_pt : curveNetwork->Vertices()) {
        double vertSum = 0;
