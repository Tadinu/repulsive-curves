...
inline double GetCoordFromBody(VertexBody6D body, int axis) {
    switch (axis) {
        ...
        case 1:
            return body.pt.position.y;
        case 2:
            return body.pt.position.z;
        case 3:
            return body.pt.tangent.x;
        case 4:
            return body.pt.tangent.y;
        case 5:
            return body.pt.tangent.z;
        default:
...
inline PosTan ReplaceCoord(PosTan body, int axis, double value) {
    switch (axis) {
        ...
        case 2:
            ...
            break;
        case 3:
            body.tangent.x = value;
            break;
        case 4:
            body.tangent.y = value;
            break;
        case 5:
            body.tangent.z = value;
            break;
        default:
            std::cout << "Invalid axis passed to ReplaceCoord: " << axis << std::endl;
            break;
    }
    return body;
}
...
inline VertexBody6D vertToBody(const PolyCurveNetwork* curves, int i) {
    const CurveVertex* p = curves->GetVertex(i);
    const Vector3 pos = p->Position();
    const Vector3 tangent = p->Tangent();

    const PosTan ptan{pos, tangent};
    const double mass = p->DualLength();
    const int globalIndex = p->GlobalIndex();

    return VertexBody6D{ptan, mass, globalIndex, BodyType::Vertex};
}

inline VertexBody6D edgeToBody(const PolyCurveNetwork* curves, int i) {
    const CurveEdge* edge = curves->GetEdge(i);

    const Vector3 pos = edge->Midpoint();
    const double mass = edge->Length();
    const Vector3 tangent = edge->Tangent();

    const PosTan ptan{pos, tangent};
    const int gIndex1 = edge->GlobalIndex();
...
BVHNode3D* CreateBVHFromCurve(const PolyCurveNetwork* curves) {
    ...
    for (int i = 0; i < nVerts; i++) {
        const VertexBody6D curBody = vertToBody(curves, i);
        // Put vertex body into full list
        verts[curBody.elementIndex] = curBody;
    }

    BVHNode3D* tree = new BVHNode3D(verts, 3, nullptr, true);
...
BVHNode3D* CreateEdgeBVHFromCurve(const PolyCurveNetwork* curves) {
    ...
    BVHNode3D* tree = new BVHNode3D(verts, 0, nullptr, true);
    tree->recomputeCentersOfMass(curves);
    BVHNode3D::globalID = 0;
    tree->recursivelyAssignIDs();
...
inline VertexBody6D meshVertToBody(const GCVertex& v, const std::shared_ptr<VertGeometry>& geom,
                                   VertIndices& indices) {
    const Vector3 pos = geom->vertexPositions[v];
    const Vector3 tan = geom->vertexNormals[v];
    const PosTan pt{pos, tan};
    const double mass = geom->vertexDualAreas[v];

    const int ind_v = indices[v];

    return VertexBody6D{pt, mass, ind_v, BodyType::Vertex};
}

BVHNode3D* CreateBVHFromMesh(const std::shared_ptr<HalfedgeMesh>& mesh,
                             const std::shared_ptr<VertGeometry>& geom) {
    ...
    const auto pair = std::pair<std::shared_ptr<HalfedgeMesh>, std::shared_ptr<VertGeometry>>(mesh, geom);
...
BVHNode3D::BVHNode3D(const std::vector<VertexBody6D>& points, int axis, BVHNode3D* root, bool splitTangents) {
    ...
    // except set the fields from that one point
        ...
        clusterIndices.insert(clusterIndices.end(), lesserNode->clusterIndices.begin(),
                              lesserNode->clusterIndices.end());
        clusterIndices.insert(clusterIndices.end(), greaterNode->clusterIndices.begin(),
                              greaterNode->clusterIndices.end());
...
void BVHNode3D::findCurveSegments(const std::vector<VertexBody6D>& points, PolyCurveNetwork* curves) {
...
double BVHNode3D::AxisSplittingPlane(const std::vector<VertexBody6D>& points, int axis) {
    ...
    for (size_t i = 0; i < nPoints; i++) {
        const double width1 = coords[i] - coords[0];
        const double width2 = (i == nPoints - 1) ? 0 : coords[nPoints - 1] - coords[i + 1];
    ...
    return (coords[splitIndex] + coords[splitIndex + 1]) / 2;
}
...
int BVHNode3D::NumElements() const {
    return numElements;
}

Vector2 BVHNode3D::viewspaceBounds(const Vector3& point) const {
    const Vector3 center = (maxCoords.position + minCoords.position) / 2;
    const Vector3 offset = (maxCoords.position - minCoords.position) / 2;

    const Vector3 toCenter = (centerOfMass - point).normalize();
    ...
    const Vector3 offset0 = corners[0] - point;
    double maxRadial = norm(offset0 - toCenter * dot(toCenter, offset0));

    for (int i = 0; i < 8; i++) {
        const Vector3 corner_offset = corners[i] - point;
        const double linearDist = dot(toCenter, corner_offset);
        minLinear = fmin(minLinear, linearDist);
        maxLinear = fmax(maxLinear, linearDist);

        const double radialDist = norm(corner_offset - toCenter * linearDist);
        maxRadial = fmax(maxRadial, radialDist);
    }

    const double linearSpread = maxLinear - minLinear;
    return Vector2{maxRadial, linearSpread};
}

bool BVHNode3D::shouldUseCell(const Vector3& vertPos) const {
    const double d = norm(centerOfMass - vertPos);
...
void BVHNode3D::accumulateVertexEnergy(double& result, const CurveVertex* i_pt,
                                       const PolyCurveNetwork* curves, double alpha, double beta) {
    if (isEmpty) {
        return;
    } else if (isLeaf) {
        ...
        if (body.type == BodyType::Vertex) {
            ...
            const CurveVertex* j = curves->GetVertex(body.elementIndex);
            ...
            result += TPESC::tpe_pair_pts(i_pt->Position(), body.pt.position, i_pt->Tangent(),
                                          i_pt->DualLength(), body.mass, alpha, beta);
        }
    } else {
        if (shouldUseCell(i_pt->Position())) {
            ...
            for (auto* child : children) {
                if (child) {
                    child->accumulateVertexEnergy(result, i_pt, curves, alpha, beta);
                }
            }
        }
    }
...
void BVHNode3D::accumulateTPEGradient(Eigen::MatrixXd& gradients, const CurveVertex* i_pt,
                                      const PolyCurveNetwork* curves, double alpha, double beta) {
    if (isEmpty) {
        return;
    } else if (isLeaf) {
        ...
        if (body.type == BodyType::Vertex) {
            ...
            std::vector<const CurveVertex*> j_pts;
            ...
            for (const CurveVertex* i_n : i_pts) {
                ...
                for (const CurveVertex* j_n : j_pts) {
        ...
        if (shouldUseCell(i_pt->Position())) {
            ...
            for (auto* child : children) {
                if (child) {
                    child->accumulateTPEGradient(gradients, i_pt, curves, alpha, beta);
                }
            }
        }
    }
...
Vector3 BVHNode3D::exactGradient(const CurveVertex* i_pt, const PolyCurveNetwork* curves, double alpha,
    ...
    if (isEmpty) {
        ...
        CurveVertex* j_pt = curves->GetVertex(body.elementIndex);
        // Don't sum if it's the same vertex
        if (j_pt == i_pt) return Vector3::zero();
...
PosTan BVHNode3D::minBound() const {
    return minCoords;
}

PosTan BVHNode3D::maxBound() const {
    return maxCoords;
}

Vector3 BVHNode3D::BoxCenter() const {
