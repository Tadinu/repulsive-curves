...
BlockClusterTree::BlockClusterTree(PolyCurveNetwork* cg, BVHNode3D* tree,
                                   double sepCoeff, double a, double b, double e) {
    curves = cg;
    alpha = a;
    beta = b;
    ...
    tree_root = tree;
    unresolvedPairs.emplace_back(ClusterPair(tree, tree));
...
void BlockClusterTree::splitInadmissibleNodes(int depth) {
    ...
    for (ClusterPair& pair : unresolvedPairs) {
        ...
        if (pair.cluster1->NumElements() == 0 || pair.cluster2->NumElements() == 0) {
            ...
            inadmissiblePairs.emplace_back(std::move(pair));
            ...
            const int cluster1_id = pair.cluster1->thisNodeID;
            admissibleByCluster[cluster1_id].emplace_back(std::move(pair));
            ...
            for (size_t i = 0; i < pair.cluster1->children.size(); i++) {
                for (size_t j = 0; j < pair.cluster2->children.size(); j++) {
                    nextPairs.emplace_back(
                        ClusterPair(pair.cluster1->children[i], pair.cluster2->children[j]));
                }
            }
        }
    }
    // Replace the inadmissible pairs by the next set
    unresolvedPairs = std::move(nextPairs);
}

bool BlockClusterTree::isPairSmallEnough(const ClusterPair& pair) {
    const int s1 = pair.cluster1->NumElements();
    const int s2 = pair.cluster2->NumElements();
    return (s1 <= 1) || (s2 <= 1) || (s1 + s2 <= 8);
}

bool BlockClusterTree::isPairAdmissible(const ClusterPair& pair, double theta) {
    if (pair.cluster1 == pair.cluster2) return false;
    const double distance = norm(pair.cluster1->centerOfMass - pair.cluster2->centerOfMass);
    const double radius1 = norm(pair.cluster1->maxBound().position - pair.cluster1->minBound().position) / 2;
    const double radius2 = norm(pair.cluster2->maxBound().position - pair.cluster2->minBound().position) / 2;
    if (distance < radius1 || distance < radius2) return false;

    const double ratio1 = pair.cluster1->nodeRatio(distance - radius2);
    const double ratio2 = pair.cluster2->nodeRatio(distance - radius1);

    const bool isAdm = fmax(ratio1, ratio2) < theta;
    return isAdm;
}

void BlockClusterTree::PrintData() const {
    std::cout << admissiblePairs.size() << " admissible pairs" << std::endl;
    std::cout << inadmissiblePairs.size() << " inadmissible pairs" << std::endl;
}

void BlockClusterTree::PrintAdmissibleClusters(std::ofstream& stream) const {
    for (const ClusterPair& p : admissiblePairs) {
        stream << p.cluster1->thisNodeID << ", " << p.cluster2->thisNodeID << std::endl;
    }
}

void BlockClusterTree::PrintInadmissibleClusters(std::ofstream& stream) const {
    for (const ClusterPair& p : inadmissiblePairs) {
        stream << p.cluster1->thisNodeID << ", " << p.cluster2->thisNodeID << std::endl;
    }
}
        ...
        if (admissibleByCluster[i].size() > 0) {
            for (auto& pair : admissibleByCluster[i]) {
                double a_IJ = SobolevCurves::MetricDistanceTerm(alpha, beta,
                                                                pair.cluster1->centerOfMass,
                                                                pair.cluster2->centerOfMass,
                                                                pair.cluster1->averageTangent,
                                                                pair.cluster2->averageTangent);
...
void BlockClusterTree::MultiplyInadmissibleLowParallel(const Eigen::VectorXd& v_mid,
                                                       Eigen::VectorXd& b_mid) const {
...
void BlockClusterTree::MultiplyInadmissibleParallel(const Eigen::MatrixXd& v_hat,
                                                    Eigen::MatrixXd& b_hat) const {
...
void BlockClusterTree::AfFullProduct(const ClusterPair& pair, const Eigen::MatrixXd& v_hat,
    ...
    for (size_t i = 0; i < pair.cluster1->clusterIndices.size(); i++) {
        const int e1index = pair.cluster1->clusterIndices[i];
        const CurveEdge* p1 = curves->GetEdge(e1index);
        const double l1 = tree_root->bvhRoot->fullMasses(e1index);

        for (size_t j = 0; j < pair.cluster2->clusterIndices.size(); j++) {
            const int e2index = pair.cluster2->clusterIndices[j];
            const CurveEdge* p2 = curves->GetEdge(e2index);
            const bool isNeighbors = (p1 == p2 || p1->IsNeighbors(p2));

            const Vector3 mid1 = p1->Midpoint();
            const Vector3 mid2 = p2->Midpoint();
            const Vector3 tan1 = p1->Tangent();
            const Vector3 tan2 = p2->Tangent();

            const double l2 = tree_root->bvhRoot->fullMasses(e2index);

            // Save on a few operations by only multiplying l2 now,
            // and multiplying l1 only once, after inner loop
            const double distTerm = SobolevCurves::MetricDistanceTerm(alpha, beta, mid1, mid2, tan1, tan2);
            const double af_ij = (isNeighbors) ? 0 : l2 * distTerm;
        ...
        const Vector3 toAdd = 2 * (a_times_one[i] * SelectRow(v_hat, e1index) - a_times_v[i]);
        AddToRow(result, e1index, toAdd);
    }
}

void BlockClusterTree::AfFullProductLow(const ClusterPair& pair, const Eigen::VectorXd& v_mid,
    ...
    for (size_t i = 0; i < pair.cluster1->clusterIndices.size(); i++) {
        ...
        for (size_t j = 0; j < pair.cluster2->clusterIndices.size(); j++) {
            ...
            // Save on a few operations by only multiplying l2 now,
            // and multiplying l1 only once, after inner loop
            const double distTerm = SobolevCurves::MetricDistanceTermLow(alpha, beta, mid1, mid2, tan1, tan2);
        ...
        // We've computed everything from row i now, so add to the results vector
        const double toAdd = 2 * (a_times_one[i] * v_mid(e1index) - a_times_v[i]);
        result(e1index) += toAdd;
    }
}

void BlockClusterTree::AfApproxProduct(const ClusterPair& pair, const Eigen::MatrixXd& v_hat,
                                       Eigen::MatrixXd& result) const {
    const long traversalStart = Utils::currentTimeMilliseconds();
    Eigen::VectorXd wf_i;
    pair.cluster1->fillClusterMassVector(wf_i);
    Eigen::VectorXd wf_j;
    pair.cluster2->fillClusterMassVector(wf_j);
    const long traversalEnd = Utils::currentTimeMilliseconds();

    traversalTime += (traversalEnd - traversalStart);

    const double a_IJ = SobolevCurves::MetricDistanceTerm(alpha, beta,
                                                          pair.cluster1->centerOfMass,
                                                          pair.cluster2->centerOfMass,
                                                          pair.cluster1->averageTangent,
                                                          pair.cluster2->averageTangent);
    // Evaluate a(I,J) * w_f(J)^T * 1(J)
    const double a_wf_1 = a_IJ * wf_j.sum();

    // Evaluate a(I,J) * w_f(J)^T * v_hat(J)
    Vector3 a_wf_J = Vector3::zero();
    ...
    for (int i = 0; i < wf_i.rows(); i++) {
        const Vector3 toAdd = wf_i[i] * 2 * (a_wf_1 * SelectRow(v_hat, pair.cluster1->clusterIndices[i]) -
                                             a_wf_J);
...
void BlockClusterTree::AfApproxProductLow(const ClusterPair& pair, const Eigen::VectorXd& v_mid,
                                          Eigen::VectorXd& result) const {
    const long traversalStart = Utils::currentTimeMilliseconds();
    Eigen::VectorXd wf_i;
    pair.cluster1->fillClusterMassVector(wf_i);
    ...
    const double a_IJ = SobolevCurves::MetricDistanceTermLow(alpha, beta,
                                                             pair.cluster1->centerOfMass,
                                                             pair.cluster2->centerOfMass,
                                                             pair.cluster1->averageTangent,
                                                             pair.cluster2->averageTangent);
    ...
    // Add in the results
    for (int i = 0; i < wf_i.rows(); i++) {
        const double toAdd = wf_i[i] * 2 * (a_wf_1 * v_mid(pair.cluster1->clusterIndices[i]) - a_wf_J);
        //CurveEdge* e_i = curves->GetEdge(pair.cluster1->clusterIndices[i]);
