...
void TPEFlowSolverSC::SetEdgeLengthScaleTarget(double scale) {
    ...
    std::cout << "Setting target length to " << targetLength << " (" << scale << " times original)" <<
        std::endl;
    double averageLength = curveNetwork->TotalLength() / curveNetwork->NumEdges();
    lengthScaleStep = averageLength / 100;
}

double TPEFlowSolverSC::CurrentEnergy(SpatialTree* root) const {
    double energy = 0;
...
double TPEFlowSolverSC::TPEnergyDirect() const {
    return TPESC::tpe_total(curveNetwork, alpha, beta);
}

double TPEFlowSolverSC::TPEnergyBH(SpatialTree* root) const {
    return SpatialTree::TPEnergyBH(curveNetwork, root, alpha, beta);
}

void TPEFlowSolverSC::FillGradientVectorDirect(Eigen::MatrixXd& gradients) const {
    TPESC::FillGradientVectorDirect(curveNetwork, gradients, alpha, beta);
}

void TPEFlowSolverSC::FillGradientVectorBH(SpatialTree* root, Eigen::MatrixXd& gradients) const {
...
void TPEFlowSolverSC::AddAllGradients(SpatialTree* tree_root, Eigen::MatrixXd& vertGradients) const {
...
void TPEFlowSolverSC::SetGradientStep(const Eigen::MatrixXd& gradient, double delta) {
    curveNetwork->positions = originalPositionMatrix - delta * gradient;
}

double TPEFlowSolverSC::LineSearchStep(const Eigen::MatrixXd& gradient, double gradDot, BVHNode3D* root,
                                       bool resetStep) {
...
double TPEFlowSolverSC::LineSearchStep(const Eigen::MatrixXd& gradient, double initGuess, int doublingLimit,
                                       double gradDot, BVHNode3D* root) {
    double delta = initGuess;
    // Save initial positions
    SaveCurrentPositions();

    const double initialEnergy = CurrentEnergy(root);
    const double gradNorm = gradient.norm();
    int numBacktracks = 0, numDoubles = 0;
    static constexpr double sigma = 0.01f;
    ...
    while (delta > ls_step_threshold) {
        ...
        const double decrease = initialEnergy - newEnergy;
        const double targetDecrease = sigma * delta * gradNorm * gradDot;
    ...
    if (delta <= ls_step_threshold) {
        std::cout << "Failed to find a non-trivial step after " << numBacktracks << " backtracks" <<
            std::endl;
...
void TPEFlowSolverSC::SetCircleStep(const Eigen::MatrixXd& P_dot, const Eigen::MatrixXd& K, double sqrt_G,
                                    double R, double alpha_delta) {
    curveNetwork->positions = originalPositionMatrix + R * (
                                  -P_dot * (sin(alpha_delta) / sqrt_G) + R * K * (1 - cos(alpha_delta)));
}

double TPEFlowSolverSC::CircleSearchStep(const Eigen::MatrixXd& P_dot, const Eigen::MatrixXd& P_ddot,
    ...
    const double G_Pd_Pd = P_dot_vec.dot(G.block(0, 0, nRows, nRows) * P_dot_vec);
    const double G_Pd_Pdd = P_dot_vec.dot(G.block(0, 0, nRows, nRows) * P_ddot_vec);

    const Eigen::VectorXd K_vec = 1.0 / G_Pd_Pd * (-P_ddot_vec + P_dot_vec * (G_Pd_Pdd / G_Pd_Pd));
    const double R = 1.0 / sqrt(K_vec.dot(G.block(0, 0, nRows, nRows) * K_vec));
    const double alpha_0 = sqrt(G_Pd_Pd);
    const double alpha_1 = 0.5 * G_Pd_Pdd / alpha_0;

    Eigen::MatrixXd K = Eigen::MatrixXd::Zero(nVerts, 3);
...
double TPEFlowSolverSC::LSBackproject(const Eigen::MatrixXd& gradient, double initGuess,
                                      const Eigen::PartialPivLU<Eigen::MatrixXd>& lu, double gradDot,
                                      BVHNode3D* root) {
    double delta = initGuess;
    int attempts = 0;
...
double TPEFlowSolverSC::ProjectSoboSloboGradient(const Eigen::PartialPivLU<Eigen::MatrixXd>& lu,
...
double TPEFlowSolverSC::BackprojectConstraints(const Eigen::PartialPivLU<Eigen::MatrixXd>& lu) {
    const int nVerts = curveNetwork->NumVertices();
    Eigen::VectorXd b = Eigen::VectorXd::Zero(constraint.NumExpectedCols() + constraint.NumConstraintRows());
    ...
    // Apply correction
    for (int i = 0; i < nVerts; i++) {
        *curveNetwork->GetVertex(i) += Vector3{corr(3 * i), corr(3 * i + 1), corr(3 * i + 2)};
    }
...
double TPEFlowSolverSC::ProjectGradient(Eigen::MatrixXd& gradients, Eigen::MatrixXd& A,
                                        Eigen::PartialPivLU<Eigen::MatrixXd>& lu) {
...
void TPEFlowSolverSC::GetSecondDerivative(SpatialTree* tree_root,
                                          Eigen::MatrixXd& projected1, double epsilon,
...
bool TPEFlowSolverSC::StepSobolevLS(bool useBH, bool useBackproj, Eigen::MatrixXd& outSobolevGradients) {
    const long start = Utils::currentTimeMilliseconds();
    ...
    std::cout << "=== Iteration " << ++iterNum << " ===" << std::endl;
    const double bh_end = Utils::currentTimeMilliseconds();

    std::cout << "  Assemble gradient " << (useBH ? "(Barnes-Hut)" : "(direct)") << ": " << (
        bh_end - bh_start) << " ms" << std::endl;
    std::cout << "  L2 gradient norm = " << l2Gradients.norm() << std::endl;

    const double length1 = curveNetwork->TotalLength();

    Eigen::MatrixXd A;
    Eigen::PartialPivLU<Eigen::MatrixXd> lu;

    const long project_start = Utils::currentTimeMilliseconds();
    // Compute the Sobolev gradient
    const double soboDot = ProjectGradient(outSobolevGradients, A, lu);
    const long project_end = Utils::currentTimeMilliseconds();
    ...
    const long end = Utils::currentTimeMilliseconds();
    std::cout << "Time = " << (end - start) << " ms" << std::endl;

    if (perfLogEnabled) {
        const double bh_time = bh_end - bh_start;
        const double mg_time = project_end - project_start;
        const double ls_time = ls_end - ls_start;
        const double bp_time = bp_end - bp_start;
        const double all_time = end - start;
...
bool TPEFlowSolverSC::StepSobolevLSIterative(double epsilon, bool useBackproj,
                                             Eigen::MatrixXd& outSobolevGradients) {
    std::cout << "=== Iteration " << ++iterNum << " ===" << std::endl;
