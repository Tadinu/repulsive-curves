...
namespace LWS {
bool CurveVertex::operator ==(const CurveVertex& other) const {
...
CurveVertex& CurveVertex::operator +=(const Vector3& other) {
    SetPosition(Position() += other);
    return *this;
}

PolyCurveNetwork::PolyCurveNetwork(const std::vector<Vector3>& ps,
                                   const std::vector<std::array<size_t, 2>>& es) {
    nVerts = ps.size();
...
PolyCurveNetwork::PolyCurveNetwork(const Eigen::MatrixXd& ps, const std::vector<std::array<size_t, 2>>& es) {
    nVerts = ps.rows();
    positions = ps;
    adjacency = std::vector<std::vector<CurveEdge*>>(nVerts);
...
PolyCurveNetwork::~PolyCurveNetwork() {
    ...
    if (constraintProjector) {
        ...
        constraintProjector = nullptr;
    }
}

void PolyCurveNetwork::InitStructs(const std::vector<std::array<size_t, 2>>& es) {
...
void PolyCurveNetwork::PrintPins() const {
...
void PolyCurveNetwork::BoundingCube(Vector3& center, double& width) const {
    Vector3 min_coords = Vector3::zero();
    Vector3 max_coords = Vector3::zero();

    // Find min and max coordinates
    for (int i = 0; i < nVerts; i++) {
        const Vector3 coords = SelectRow(positions, i);
    ...
    const Vector3 diff = vector_abs(max_coords - min_coords);
    width = fmax(diff.x, fmax(diff.y, diff.z)) * 1.05;
}

Vector3 PolyCurveNetwork::Barycenter() const {
    Vector3 center{0, 0, 0};
    double totalMass = 0;
    for (int i = 0; i < nVerts; i++) {
        const double m = vertices[i]->DualLength();
...
double PolyCurveNetwork::TotalLength() const {
    double length = 0;
    for (size_t i = 0; i < edges.size(); i++) {
...
Vector3 PolyCurveNetwork::AreaVector() const {
    const int nVerts = NumVertices();
    Vector3 sum{0, 0, 0};

    for (int i = 0; i < nVerts; i++) {
        const CurveVertex* v_i = GetVertex(i);
        const CurveVertex* v_next = v_i->edge(0)->Opposite(v_i);
...
PolyCurveNetwork* PolyCurveNetwork::Subdivide() const {
    ...
    for (int i = 0; i < nEdges; i++) {
        const CurveEdge* e_i = GetEdge(i);

        const CurveVertex* prev = e_i->prevVert;
        const CurveVertex* next = e_i->nextVert;
        ...
        const Vector3 midpoint = (prev->Position() + next->Position()) / 2;
        const size_t midIndex = newPositions.size();
...
PolyCurveNetwork* PolyCurveNetwork::Coarsen(MatrixProjectorOperator* op, bool doEdgeMatrix) const {
