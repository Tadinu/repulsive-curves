...
namespace LWS {
MeshObstacle::MeshObstacle(const std::shared_ptr<HalfedgeMesh>& m,
                           const std::shared_ptr<VertexPositionGeometry>& geom,
...
void MeshObstacle::AddGradient(const PolyCurveNetwork* curves, Eigen::MatrixXd& gradient) const {
...
double MeshObstacle::ComputeEnergy(const PolyCurveNetwork* curves) const {
    int nVerts = curves->NumVertices();
    double sumE = 0;
    for (int i = 0; i < nVerts; i++) {
        Vector3 pos = curves->GetVertex(i)->Position();
...
inline double bodyEnergy(BVHNode3D* bvh, const Vector3& point, double p) {
...
double MeshObstacle::AccumulateEnergy(BVHNode3D* node, const Vector3& point) const {
    if (node->IsEmpty()) {
        return 0;
    } else if (node->IsLeaf()) {
...
inline Vector3 bodyForce(BVHNode3D* bvh, const Vector3& point, double p) {
    const Vector3 center = bvh->centerOfMass;
    const double mass = bvh->totalMass;

    Vector3 toPoint = center - point;
    const double distance = toPoint.norm();
    toPoint /= distance;
...
Vector3 MeshObstacle::AccumulateForce(BVHNode3D* node, const Vector3& point) const {
    if (node->IsEmpty()) {
