...
class ClusterPair {
public:
    ClusterPair(BVHNode3D* c1 = nullptr, BVHNode3D* c2 = nullptr, int d = 0)
        : cluster1(c1), cluster2(c2), depth(d) {
    }

    BVHNode3D* cluster1 = nullptr;
    BVHNode3D* cluster2 = nullptr;
    int depth = 0;
};
...
public:
    ...
    BlockClusterTree(PolyCurveNetwork* cg, BVHNode3D* tree, double sepCoeff, double a, double b,
                     double e = 0.0);
    ~BlockClusterTree();
    ...
    static bool isPairAdmissible(const ClusterPair& pair, double coeff);
    static bool isPairSmallEnough(const ClusterPair& pair);
    ...
    void PrintAdmissibleClusters(std::ofstream& stream) const;
    void PrintInadmissibleClusters(std::ofstream& stream) const;
    ...
    void AfFullProduct(const ClusterPair& pair, const Eigen::MatrixXd& v_hat, Eigen::MatrixXd& result) const;
    void AfFullProductLow(const ClusterPair& pair, const Eigen::VectorXd& v_mid,
                          Eigen::VectorXd& result) const;
    void AfApproxProduct(const ClusterPair& pair, const Eigen::MatrixXd& v_hat,
                         Eigen::MatrixXd& result) const;
    void AfApproxProductLow(const ClusterPair& pair, const Eigen::VectorXd& v_mid,
                            Eigen::VectorXd& result) const;
...
private:
    ...
    BlockTreeMode mode = BlockTreeMode::MatrixOnly;
    int nVerts = 0;
    double alpha = 0., beta = 0., separationCoeff = 0.;
    double epsilon = 0.;
    PolyCurveNetwork* curves = nullptr;
    BVHNode3D* tree_root = nullptr;
