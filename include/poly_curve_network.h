...
public:
    PolyCurveNetwork(const std::vector<Vector3>& ps, const std::vector<std::array<size_t, 2>>& es);
    PolyCurveNetwork(const Eigen::MatrixXd& ps, const std::vector<std::array<size_t, 2>>& es);
    ~PolyCurveNetwork();

    void InitStructs(const std::vector<std::array<size_t, 2>>& es);
    void FindComponents();
    ...
    inline double BendingAngle(int i) const {
        ...
        const CurveVertex* v_prev = v_i->edge(0)->Opposite(v_i);
        const CurveVertex* v_next = v_i->edge(1)->Opposite(v_i);

        const Vector3 e_prev = (v_i->Position() - v_prev->Position()).normalize();
        const Vector3 e_next = (v_next->Position() - v_i->Position()).normalize();

        const double sin_angle = norm(cross(e_prev, e_next));
        const double cos_angle = dot(e_prev, e_next);
    ...
    NullSpaceProjector* constraintProjector = nullptr;
